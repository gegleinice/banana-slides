...
// Next.js 壳模式：所有后端请求通过 /api/backend/* 重写到 Python 后端。
// 配合 next.config.ts 中的 rewrite：/api/backend/:path* → BANANA_BACKEND_URL/:path*
// 因此 endpoint 里继续写 '/api/access-code/check' 等路径即可，
// 实际请求为 /api/backend/api/access-code/check → {backend}/api/access-code/check
...
export const getImageUrl = (path?: string, timestamp?: string | number): string => {
  ...
  // 使用相对路径（确保以 / 开头），并补上 /api/backend 前缀以命中后端重写
  const rel = path.startsWith('/') ? path : '/' + path;
  let url = rel.startsWith('/api/backend') ? rel : `/api/backend${rel}`;
